  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Tween<double> top = Tween<double>(
      begin: selectedItemOffset.clamp(
          0.0, size.height - _kMenuItemHeight) as double,
      end: 0.0,
    );

    final Tween<double> bottom = Tween<double>(
      begin: (top.begin + _kMenuItemHeight).clamp(
          _kMenuItemHeight, size.height) as double,
      end: size.height,
    );
...
class _DropdownScrollBehavior extends ScrollBehavior {
  ...
  Widget buildViewportChrome(BuildContext context, Widget child,
      AxisDirection axisDirection) => child;
...
class _DropdownMenuItemButtonState<T>
  ...
  void _handleOnTap() {
    final DropdownMenuItem<T> dropdownMenuItem = widget.route.items[widget
        .itemIndex].item;
  ...
  static final Map<LogicalKeySet, Intent> _webShortcuts = <LogicalKeySet,
      Intent>{
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.itemIndex == widget.route.selectedIndex) {
      ...
      final double start = (0.5 + (widget.itemIndex + 1) * unit).clamp(
          0.0, 1.0) as double;
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    final double maxHeight = math.max(
        0.0, constraints.maxHeight - 2 * _kMenuItemHeight);
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    final _MenuLimits menuLimits = route.getMenuLimits(
        buttonRect, size.height, route.selectedIndex);
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  _MenuLimits getMenuLimits(Rect buttonRect, double availableHeight,
    ...
    final double bottomLimit = math.max(
        availableHeight - _kMenuItemHeight, buttonBottom);
...
class _DropdownButtonState<T> extends State<DropdownButton<T>>
  ...
  void _handleTap() {
    ...
    final TextDirection textDirection = Directionality.of(context);
    final EdgeInsetsGeometry menuMargin = ButtonTheme
        .of(context)
        .alignedDropdown
        ? _kAlignedMenuMargin
        : _kUnalignedMenuMargin;

    final List<_MenuItem<T>> menuItems = List<_MenuItem<T>>(
        widget.items.length);
    ...
    Navigator.push(context, _dropdownRoute).then<void>((
  ...
  double get _denseButtonHeight {
    final double fontSize = _textStyle.fontSize ?? Theme
        .of(context)
        .textTheme
        .subtitle1
        .fontSize;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.hint != null || (!_enabled && widget.disabledHint != null)) {
      Widget displayedHint = _enabled ? widget.hint : widget.disabledHint ??
          widget.hint;
      ...
      innerItemsWidget = IndexedStack(
        ...
        children: widget.isDense ? items : items.map((Widget item) {
          return widget.itemHeight != null
              ...
              : Column(
              mainAxisSize: MainAxisSize.min, children: <Widget>[item]);
        }).toList(),
      );
    }
    ...
    if (!DropdownButtonHideUnderline.at(context)) {
      final double bottom = (widget.isDense || widget.itemHeight == null)
          ? 0.0
          : 8.0;
      result = Stack(
        children: <Widget>[
  ...
  "There should be exactly one item with [DropdownButton]'s value: "
      ...
      'with the same value',
        ...
        builder: (FormFieldState<T> field) {
          final _DropdownButtonFormFieldState<
              T> state = field as _DropdownButtonFormFieldState<T>;
          final InputDecoration decorationArg = decoration ??
              InputDecoration(focusColor: focusColor);
